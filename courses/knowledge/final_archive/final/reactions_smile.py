Na + Cl2 → NaCl
Fe + Cl2 → FeCl3
Cu + Cl2 → CuCl2
Cl2 + H2O → HCl + HClO
MnO2 + 4HCl = MnCl2 + Cl2↑ + 2H2O
HCl + KMnO4 → KCl + MnCl2 + H2O + Cl2↑
NaCl + H2O → Cl2↑ + H2↑ + NaOH
K + Cl2 → KCl
Na + Cl2 → NaCl
Al + Cl2 → AlCl3
Ca + Cl2 → CaCl2
FeCl2 + Cl2 → FeCl3
Cl2 + KBr → KCl + Br2
Cl2 + NaBr → NaCl + Br2
NaCl + H2SO4 -> NaHSO4 + HCl↑
NaCl + H2SO4 -> Na2SO4 + HCl↑
H2 + Cl2 → HCl
KCl + H2SO4 → KHSO4 + HCl
KCl + H2SO4 → K2SO4 + HCl
CaCl2 + H2SO4 → Ca(HSO4)2 + HCl
CaCl2 + H2SO4 → CaSO4 + HCl
Zn + HCl → ZnCl2 + H2↑
Fe + HCl → FeCl2 + H2↑
Al + HCl → AlCl3 + H2↑
Ca + HCl → CaCl2 + H2↑
Na + HCl → NaCl + H2↑
K + HCl → KCl + H2↑
Mg + HCl → MgCl2 + H2↑
CuO + HCl → CuCl2 + H2O
ZnO + HCl → ZnCl2 + H2O
MgO + HCl → MgCl2 + H2O
MnO2 + HCl → MnCl2 + Cl2↑ + H2O
Al2O3 + HCl → AlCl3 + H2O
Al(OH)3 + HCl → AlCl3 + H2O
NaOH + HCl → NaCl + H2O
KOH + HCl → KCl + H2O
Zn(OH)2 + HCl → ZnCl2 + H2O
Cu(OH)2 + HCl → CuCl2 + H2O
Cl2 + KOH → KClO3 + KCl + H2O
Cl2 + NaOH → NaClO + NaCl + H2O
AgNO3 + HCl → HNO3 + AgCl↓
CaCO3 + HCl → CaCl2 + H2O + CO2↑
MgCO3 + HCl → MgCl2 + H2O + CO2↑
BaSO3 + HCl → BaCl2 + H2SO3
AgNO3 + NaCl → NaNO3 + AgCl↓
AgNO3 + KCl → KNO3 + AgCl↓
H2S + HClO3 → HCl + H2SO4
Cl2 + NaOH → NaCl + NaClO + H2O
Cl2 + KOH → KCl + KClO + H2O
Cl2 + KOH → KCl + KClO3 + H2O
KClO3 → KCl + O2↑
KClO3 → KCl + KClO4
Ca(OH)2 + Cl2 → CaOCl2 + H2O
Al + Br2 → AlBr3
Al + I2 → AlI3
H2 + Br2 → HBr
H2 + I2 → HI
HI → H2 + I2
Br2 + NaI → NaBr + I2
Cl2 + NaBr → NaCl + Br2
H2 + F2 → HF
SiO2 + HF → SiF4 + H2O
F2 + H2O → HF + O2↑
Ca + O2 → CaO
Al + O2 → Al2O3
Fe + O2 → Fe2O3
Fe + O2 → Fe3O4
Fe + O2 → FeO
Zn + O2 → ZnO
Cu + O2 → CuO
Mg + O2 → MgO
Na + O2 → Na2O
K + O2 → K2O
S + O2 → SO2
SO2 + O2 → SO3
C + O2 → CO2
N2 + O2 → NO
H2 + O2 → H2O
NH3 + O2 → NO + H2O
KClO3 → KCl + O2↑
KMnO4 → K2MnO4 + MnO2 + O2↑
H2O →H2↑ + O2↑
Fe + S FeS
Zn + S → ZnS
Cu + S → CuS
Hg + S → HgS
H2 + S → H2S
KNO3 + S + C → K2S + N2 + CO2
H2S + O2 → SO2 + H2O
H2S + O2 → S + H2O
H2S + Cl2 + H2O → H2SO4 + HCl
H2S + Pb(NO3)2 → PbS ↓ + HNO3
Na2S + Pb(NO3)2 → PbS ↓ + NaNO3
H2S + CuSO4 → CuS↓ + H2SO4
FeS + HCl → FeCl2 + H2S
ZnS + HCl → ZnCl2 + H2S
SO2 + CaO → CaSO3
SO2 + NaOH → Na2SO3 + H2O
H2O + SO2 → H2SO3
H2S + SO2 → S + H2O
SO3 + H2O → H2SO4
FeS2 + O2 → Fe2O3 + SO2↑
ZnS + O2 → ZnO + SO2↑
CuS + O2 → CuO + SO2↑
SO2 + O2 → SO3↑
SO2 + H2O → H2SO3
SO2 + NaOH → NaHSO3
NaOH + H2SO4 → Na2SO4 + H2O
KOH + H2SO4 → K2SO4 + H2O
Ba(OH)2 + H2SO4 → BaSO4 + H2O
Mg(OH)2 + H2SO4 = MgSO4 + 2H2O
Fe(OH)3 + H2SO4 → Fe2(SO4)3 + H2O
CuO + H2SO4 → CuSO4 + H2O
ZnO + H2SO4 → ZnSO4 + H2O
Zn + H2SO4 → ZnSO4 + H2
Fe + H2SO4 → FeSO4 + H2
Na + H2SO4 → Na2SO4 + H2
K + H2SO4 → K2SO4 + H2
Cu + H2SO4 → CuSO4 + SO2↑ + H2O
3 Mg + 4H2SO4 = 3MgSO4 + S + H2O
C + H2SO4 → CO2↑ + SO2↑ + H2O
CaCO3 + H2SO4 → CaSO4 + H2O + CO2
K2CO3 + H2SO4 → K2SO4 + H2O + CO2
Ba(NO3)2 + H2SO4 → Ba2SO4 + HNO3
H2SO4 + H2S → S↓ + SO2↑ + H2O
H2SO4 + BaCl2 → BaSO4↓ + HCl
Na2SO4 + BaCl2 → BaSO4↓ + NaCl
Na2SO3 + BaCl2 → BaSO3↓ + NaCl
K2SO4 + BaCl2 → BaSO4↓ + KCl
K2SO3 + BaCl2 → BaSO3↓ + Kcl
Na2SO4 + KCl → K2SO4↓ + NaCl
Fe + CuSO4 → FeSO4 + Cu
Zn + CuSO4 → ZnSO4 + Cu
CuSO4 + KOH → Cu(OH)2↓ + K2SO4
CuSO4 + NaOH → Cu(OH)2↓ + Na2SO4
Zn + H2SO4 → ZnSO4 + S + H2O
Mg + H2SO4 → MgSO4 + H2S + H2O
Ag + H2SO4 → Ag2SO4 + SO2 + H2O
S + H2SO4 → SO2 + H2O
FeS2 + O2 Fe2O3 + SO2
H2SO3 + Cl2 + H2O → H2SO4 + HCl
H2SO3 + Br2 + H2O → H2SO4 + HBr
Na2SO3 + H2SO4 → Na2SO4 + H2O + SO2↑
Na2CO3 + H2SO4 → Na2SO4 + H2O + CO2↑
H2 + CuO → Cu + H2O
H2 + ZnO → Zn + H2O
Fe2O3 + 3H2 = 2Fe + 3H2O
CaCO3 → CaO + CO2
NaNO3 → NaNO2 + O2↑
N2 + 3H2 = 2NH3
Zn(OH)2 + 2HCl = ZnCl2 + 2H2O
Cu(OH)2 + HNO3 → Cu(NO3)2 + H2O
C + CuO → Cu + CO2
C + H2O → CO + H2
C + H2O → CO2 + H2
Al + H2O → Al(OH)3↓ + H2↑
Al + Fe2O3 → Al2O3 + Fe
Al2O3 → Al + O2↑
Al2O3 + NaOH → NaAlO2 + H2O
Al(OH)3 → Al2O3 + H2O
Al(OH)3 + NaOH → NaAlO2 + H2O
Ca + 2H2O = Ca(OH)2 + H2↑
Ca + HNO3 (dung dịch loãng)→ Ca(NO3)2 + NH4NO3 + H20
CaCl2 → Ca + Cl2↑
Na + H2O → NaOH + H2↑
NaCl → Na + Cl2↑
NaOH → Na + O2↑ + H20
NaI + Br2 → NaBr + I2
KI + Br2 → KBr + I2
Fe + H2O → Fe3O4 + H2↑
Fe + H2O → FeO + H2↑
Fe + H2O + O2 → Fe(OH)3
Fe + HNO3 → Fe(NO3)3 + N20 + H20
Fe(OH)3 + HNO3 → Fe(NO3)3 + H20
CO2 + Ca(OH)2 = CaCO3 + H2O
(NH4)2SO4 → NH3 + H2SO4
FeSO4 + KMnO4 + H2SO4 → Fe2(SO4)3 + MnSO4 + K2SO4 + H2O
KMnO4 → K2MnO4 + MnO2 + O2
HNO3 → NO2 + O2 + H2O
NH4NO2 → N2 + H2O
NH4NO3 → N2 + O2 + H2O
NO2 + NaOH → NaNO2 + NaNO2 + H2O
