...
- [2. Giới thiệu](#2-giới-thiệu)
  - [2.1. Bối cảnh và Lý do chọn đề tài](#21-bối-cảnh-và-lý-do-chọn-đề-tài)
  - [2.2. Mục tiêu của Dự án](#22-mục-tiêu-của-dự-án)
  - [2.3. Phạm vi Nghiên cứu](#23-phạm-vi-nghiên-cứu)
- [3. Cơ sở Lý thuyết và Kiến trúc Hệ thống](#3-cơ-sở-lý-thuyết-và-kiến-trúc-hệ-thống)
  - [3.1. Mô hình Databricks Lakehouse](#31-mô-hình-databricks-lakehouse)
  - [3.2. Công nghệ Delta Lake](#32-công-nghệ-delta-lake)
  - [3.3. Công nghệ Apache Spark](#33-công-nghệ-apache-spark)
- [4. Triển khai ETL: Kiến trúc Medallion](#4-triển-khai-etl-kiến-trúc-medallion)
  - [4.1. Lớp Bronze: Nhập liệu Thô (Raw Ingestion)](#41-lớp-bronze-nhập-liệu-thô-raw-ingestion)
    - [4.1.1. Kỹ thuật Web Scraping với Scrapy](#411-kỹ-thuật-web-scraping-với-scrapy)
    - [4.1.2. Các vấn đề Đạo đức và Tuân thủ](#412-các-vấn-đề-đạo-đức-và-tuân-thủ)
  - [4.2. Lớp Silver: Chuyển đổi và Làm sạch (Transformation \& Cleansing)](#42-lớp-silver-chuyển-đổi-và-làm-sạch-transformation--cleansing)
    - [4.2.1. Batch Inference trên Databricks: Phân loại Địa điểm Thành phố Việt Nam](#421-batch-inference-trên-databricks-phân-loại-địa-điểm-thành-phố-việt-nam)
    - [4.2.2. Tối ưu hóa Hiệu năng](#422-tối-ưu-hóa-hiệu-năng)
  - [4.3. Kiểm thử Chất lượng](#43-kiểm-thử-chất-lượng)
    - [4.3.1. Kiểm thử Logic Chuyển đổi (Unit Testing)](#431-kiểm-thử-logic-chuyển-đổi-unit-testing)
    - [4.3.2. Kiểm soát Chất lượng Phân loại AI](#432-kiểm-soát-chất-lượng-phân-loại-ai)
  - [4.4. Lớp Gold: Tổng hợp Nghiệp vụ (Business Aggregation)](#44-lớp-gold-tổng-hợp-nghiệp-vụ-business-aggregation)
  - [4.5. Databricks Declarative Pipelines: Tự động hóa Thế hệ mới](#45-databricks-declarative-pipelines-tự-động-hóa-thế-hệ-mới)
...
- [6. Trực quan hóa và Phân tích Dữ liệu](#6-trực-quan-hóa-và-phân-tích-dữ-liệu)
  - [6.1. Dashboard Tổng quan Thị trường (Market Overview)](#61-dashboard-tổng-quan-thị-trường-market-overview)
  - [6.2. Dashboard Chi tiết và Tương tác (Interactive Filtering)](#62-dashboard-chi-tiết-và-tương-tác-interactive-filtering)
...
# 3. Cơ sở Lý thuyết và Kiến trúc Hệ thống

## 3.1. Mô hình Databricks Lakehouse
...
### 4.1.1. Kỹ thuật Web Scraping với Scrapy

Dữ liệu được thu thập bằng **Scrapy**, một framework Python chuyên dụng cho web scraping. Scrapy được lựa chọn vì kiến trúc bất đồng bộ (asynchronous) cho phép gửi hàng trăm request cùng lúc mà không cần chờ từng phản hồi, giúp tăng tốc độ thu thập dữ liệu đáng kể so với các công cụ như Selenium.

Trình crawler được thiết kế để trích xuất các trường thông tin quan trọng từ 1900.com.vn thông qua **ItemLoader** và **Field definitions**, đảm bảo dữ liệu được thu thập một cách có cấu trúc.

**Quy trình xử lý:**
1. **Thu thập:** Crawler tải dữ liệu thô và xử lý sơ bộ (loại bỏ tags HTML, chuẩn hóa khoảng trắng).
2. **Lưu trữ:** Dữ liệu JSON thô được ghi vào Databricks DBFS.
3. **Ingestion:** Sử dụng PySpark để đọc và chuyển dữ liệu vào bảng Delta lớp **Bronze** trong **Unity Catalog** với chế độ `append`. Tại đây, nguyên tắc **Schema-on-Read** được áp dụng để lưu giữ nguyên vẹn dữ liệu gốc, bao gồm cả lịch sử các lần chạy.

**Mã nguồn định nghĩa cấu trúc dữ liệu (Item Definition):**
...
### 4.1.2. Các vấn đề Đạo đức và Tuân thủ

Dự án cam kết tuân thủ nghiêm ngặt các chuẩn mực đạo đức trong thu thập dữ liệu để tránh gây ảnh hưởng đến hệ thống mục tiêu và tôn trọng quyền sở hữu dữ liệu.

**Các biện pháp cụ thể:**

1. **Tuân thủ Robots.txt:** Crawler tự động kiểm tra và tuân thủ các chỉ thị `User-agent` và `Disallow` của trang web.
2. **Giới hạn Tốc độ (Rate Limiting):**
   - Áp dụng độ trễ ngẫu nhiên (3-7 giây) giữa các request.
   - Giới hạn số lượng request đồng thời thấp để không chiếm dụng băng thông, tránh gây quá tải server (DDoS).
3. **Cơ chế Back-off:** Tự động tạm dừng và tăng thời gian chờ theo cấp số nhân nếu gặp lỗi 429 (Too Many Requests) hoặc 503.
4. **Bảo vệ dữ liệu cá nhân:** Chỉ thu thập thông tin công khai về công việc và công ty, chủ động loại bỏ hoặc làm mờ các thông tin cá nhân nhạy cảm (PII) nếu vô tình xuất hiện. Dữ liệu chỉ phục vụ mục đích nghiên cứu giáo dục, không thương mại hóa.
...
### 4.2.1. Batch Inference trên Databricks: Phân loại Địa điểm Thành phố Việt Nam
...
### 4.3.1. Kiểm thử Logic Chuyển đổi (Unit Testing)
...
### 4.3.2. Kiểm soát Chất lượng Phân loại AI
...
## 4.4. Lớp Gold: Tổng hợp Nghiệp vụ (Business Aggregation)
...
## 4.5. Databricks Declarative Pipelines: Tự động hóa Thế hệ mới
...
## 6.1. Dashboard Tổng quan Thị trường (Market Overview)

![alt text](images/final-3.png)
_Hình 3. Dashboard tổng quan thị trường lao động trong tháng vừa qua_
...
## 6.2. Dashboard Chi tiết và Tương tác (Interactive Filtering)

![alt text](images/final-2.png)
_Hình 4. Dashboard chi tiết với bộ lọc tương tác theo Công ty_
...
- **KPIs Cụ thể:** Hiển thị các chỉ số vi mô cho thực thể được chọn:
  - _Area Availability:_ Tỷ lệ phân bố công việc của công ty giữa các chi nhánh.
  - _Company Rating:_ Điểm đánh giá trung bình của công ty (3.5).
  - _Average Experience:_ Yêu cầu kinh nghiệm trung bình (0.67 năm).
...
1. Armbrust, M., et al. (2020). _Delta Lake: High-Performance ACID Table Storage over Cloud Object Stores_. PVLDB.
2. Databricks. (2025). _The Data Lakehouse Architecture: Building the Modern Data Stack_.
3. Kimball, R., & Ross, M. (2013). _The Data Warehouse Toolkit: The Definitive Guide to Dimensional Modeling_. Wiley.
4. Scrapy Developers. (2025). _Scrapy 2.13 Documentation_.
5. Apache Spark. (2025). _Apache Spark Documentation_. Retrieved from <https://spark.apache.org/docs/latest/>
6. Chambers, B., & Zaharia, M. (2018). _Spark: The Definitive Guide: Big Data Processing Made Simple_. O'Reilly Media.
7. Damji, J. S., Wenig, B., Das, T., & Lee, D. (2020). _Learning Spark: Lightning-Fast Data Analytics_. O'Reilly Media.
8. Delta Lake. (2025). _Delta Lake Documentation_. Retrieved from <https://docs.delta.io/latest/index.html>
9. Databricks. (2025). _Databricks Unity Catalog Documentation_. Retrieved from <https://docs.databricks.com/data-governance/unity-catalog/index.html>
10. Databricks. (2025). _Databricks Workflows Documentation_. Retrieved from <https://docs.databricks.com/workflows/index.html>
